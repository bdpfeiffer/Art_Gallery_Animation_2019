...
earTriangulation    triangulation = new earTriangulation();
...
void draw(){
  ...
  textRHC( "p: Print new edges list", 10, height-60 );
  textRHC( "c: Clear Polygon", 10, height-80 );
  textRHC( "s: Save Image", 10, height-100 );
  textRHC( "r: Ready to Begin", 10, height-120);
  ...
  textRHC( "ready?: " + ready, 550, 100);
  textRHC( "counter: " + counter, 550, 120);
...
void keyPressed(){
  ...
  if( key == 'r' ) {ready = true; counter = 0; poly.readyToBegin();}
  if( key == 'c' ){ points.clear(); poly = new Polygon(); ready = false; counter = 0;}
  if( key == 'p' ) poly.printNewEdges();//showPotentialDiagonals = !showPotentialDiagonals;
